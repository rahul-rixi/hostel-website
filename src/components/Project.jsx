  ...
  return (
    <>
      <div className="max-w-[90%] mx-auto">
        <nav
          className={`h-16 w-full flex justify-between items-center px-8 backdrop-blur-lg shadow-lg z-50 fixed top-0 left-0 right-0 transition-all duration-300 ease-in-out ${navbarVisible ? 'translate-y-0' : '-translate-y-full'}`}
        >
          <NavLink to="/" className="text-3xl text-white font-bold tracking-wider flex items-center">
            ...
            <span className="text-gray-200 ml-1">Connect</span>
          </NavLink>
            ...
            {['Home', 'Facilities', 'Gallery', 'Contact', 'Login'].map((link) => (
              <div key={link} className="relative group">
                <NavLink
                  ...
                  className="block px-4 py-3 text-gray-800 font-semibold hover:text-indigo-500 transition-all duration-300"
                  style={({ isActive }) => ({
                    color: isActive ? '#4C51BF' : '#333333', // Active color (indigo) and default text (dark gray)
                  })}
                >
      ...
      <div
        className={`fixed top-0 left-0 h-screen bg-transparent backdrop-blur-md text-white z-50 transform ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out`}
        style={{ width: '80%' }}
      >
        <ul className="flex flex-col gap-8 mt-10 px-6">
          {['Home', 'Facilities', 'Gallery', 'Contact', 'Login'].map((link, index) => (
            <li
              key={link}
              className={`text-lg font-semibold text-gray-800 cursor-pointer transform hover:scale-105 transition-all duration-300 ease-out ${isMenuOpen ? 'opacity-100' : 'opacity-0'}`}
              style={{
